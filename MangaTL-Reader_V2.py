...
def _ocr_gemini_vision(image_bytes: bytes, lang: str, key: str, model: str) -> list:
    ...
    # Gemini Vision bills per 768×768 tile.  A typical MangaDex page (1600×2300)
    # costs ~9 tiles ≈ 2300 tokens.  Capping at 800px wide drops it to ~4 tiles
    # ≈ 1030 tokens — roughly halved — with no meaningful OCR quality loss since
    # text recognition doesn't need pixel-perfect full resolution.
    try:
        ...
        # If resize fails for any reason, fall back to the raw bytes unchanged
    ...
    prompt = (
        ...
        f'"x1":<left edge %>,"y1":<top edge %>,"x2":<right edge %>,"y2":<bottom edge %>}},...]\n\n'
        f"Rules:\n"
        ...
        f"- x1/y1/x2/y2 = bounding box corners as percentage of image size (estimate is fine).\n"
    ...
    try:
        ...
        for item in items:
            ...
            if not t:
                continue
            # Use the bounding box the model returned; fall back to a small
            # synthesised box centred on cx/cy only when the model omits it.
            # The hardcoded 16×10% fallback was the old behaviour — it produced
            # visibly wrong correction-UI overlays for large bubbles and tiny SFX.
            x1 = item.get("x1")
            y1 = item.get("y1")
            x2 = item.get("x2")
            y2 = item.get("y2")
            if None not in (x1, y1, x2, y2):
                try:
                    ...
                    box = [cx - 8.0, cy - 5.0, cx + 8.0, cy + 5.0]
            else:
                box = [cx - 8.0, cy - 5.0, cx + 8.0, cy + 5.0]
            out.append({
                "text": t,
...
def _translate_deepseek(api_key: str, payload: dict):
    ...
    try:
        ...
        # Re-raise Flask abort() / HTTPException so errors reach the client.
        # Without this, abort(422, ...) inside the try block would be caught here
        # and silently replaced by a raw pass-through of the DeepSeek response.
        if hasattr(_exc, "code"):  # werkzeug HTTPException carries .code
            raise
        # Unexpected response shape — pass through raw.
        return Response(r.content, status=r.status_code,
                        content_type=r.headers.get("Content-Type", "application/json"))
